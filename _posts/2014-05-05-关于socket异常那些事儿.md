...
openfire自带了心跳机制，是通过在服务端设置

MINA框架本身提供了空闲检测功能，这项功能可检测客户端口建立了TCP/IP连接、却不发送任何消息的情况。

当我们设置了第一项中的时长，openfire会调用MINA的session.setIdleTime()方法，在客户端口连接经过指定时长未发送任何消息的情况下触发sessionIdle事件，由sessionIdle()方法处理。